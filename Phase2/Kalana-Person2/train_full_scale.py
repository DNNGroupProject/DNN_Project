"""Full-scale SegFormer-B0 training for the paper numbers (Person 2, Phase 2).

Logic is Person 3's `train_segformer_smoke.py` (same `train_variant`, same
Attention Consistency Loss path). Differences, all local to this file:

- Defaults are the audited split / 20 epochs / batch 16, not the smoke 400/8/4.
- Checkpoints and logs go under this folder, not `Dinura-Person3/`.
- Model, images, and masks are moved onto CUDA when a GPU exists. Person 3's
  Colab notebook patched `to_model_input` / `build_segformer` but left the
  mask tensor on CPU — that would break `dice_bce` on GPU, so both sides of
  each batch are moved here without editing the shared module.
"""
