...
(3576/766/766 split, seed 42, 20 epochs), the proposed method raises AAMO
from 0.0334 to 0.5752 ($\sim$17$\times$) relative to a vanilla SegFormer-B0
baseline trained on the same split, while Dice (0.8743$\to$0.8690) and IoU
(0.7766$\to$0.7684) are essentially unchanged, alongside a full-scale-trained
U-Net baseline (Dice 0.8615, IoU 0.7568) on the same held-out test set. A
complete multi-seed ablation study is ongoing.
\end{abstract}
...
cost), Gaussian target smoothing $\sigma = 8$, $\lambda_2 = 0.3$, and
$L_{att}$ using the MSE formulation, seed 42. Reported checkpoints are each
variant's best-validation epoch (vanilla: epoch 5; attention-consistency:
epoch 3) rather than the final epoch, since both variants overfit under this
training budget (training Dice reaches $\sim$0.90 by epoch 20 while
validation Dice peaks earlier). Both the MSE and KL-divergence formulations
of $L_{att}$ are implemented and unit-tested, but only MSE has been
evaluated on trained checkpoints so far.
...
U-Net and both SegFormer-B0 variants, all on the same 3576/766/766
held-out test set. AAMO increases approximately $17\times$
(0.0334 $\to$ 0.5752) with the attention-consistency variant, while Dice and
IoU are essentially unchanged (vanilla marginally ahead: 0.8743 vs 0.8690
Dice), suggesting the attention supervision does not cost segmentation
accuracy at full scale. This differs from our earlier 400/60/60, 8-epoch CPU
smoke run, where the attention loss had also slightly \emph{improved}
Dice/IoU (0.8017$\to$0.8191, 0.6690$\to$0.6936); that small-scale accuracy
gain did not hold once trained at full scale, though the AAMO direction was
consistent at both scales (smoke: 0.0144$\to$0.432). We additionally report
...
seed 42): U-Net and SegFormer-B0 variants share this held-out test set.
SegFormer checkpoints are each variant's best-validation epoch (vanilla
epoch 5, attention-consistency epoch 3; both variants overfit past this
point). DeepLabV3+ remains a 200-image smoke run, an optional extra
baseline not yet run at full scale.}
\label{tab:results}
...
SegFormer-B0 + $L_{att}$ & 0.8690 & 0.7684 & 0.8690 & \textbf{0.5752} \\
\bottomrule
\end{tabular}
\end{table}
...
variants, but both SegFormer variants overfit under this budget (best
validation Dice at epoch 5/3 of 20), so Table~\ref{tab:results} reports
early-stopped checkpoints rather than fully converged ones, and no run has
yet been repeated with a different seed. DeepLabV3+ remains a 200-image
...
\section{Discussion}
The large AAMO gain relative to an essentially flat Dice/IoU change
suggests attention faithfulness and segmentation accuracy are, at least
partially, separable objectives: supervising attention does not appear to
cost accuracy, and this now holds at full scale (5{,}108 images, 20 epochs)
as well as at the earlier smoke scale, though a multi-seed ablation is
still required before drawing firm conclusions. Both the MSE and
KL-divergence formulations of $L_{att}$ are implemented; only MSE has been
evaluated so far.
...
caveat. Both SegFormer variants also overfit past their best-validation
epoch under this training budget (Section~4), so the reported numbers are
early-stopped rather than fully converged. (2) \emph{Single seed.} No run
has been repeated with a different seed, so the reported numbers carry
...
core hypothesis: AAMO rises $\sim$17$\times$ (0.0334$\to$0.5752) while
segmentation accuracy is essentially unchanged. Future work will run the
complete multi-seed ablation study, compare the MSE and KL-divergence loss
formulations, tune $\lambda_2$ via a validation sweep, address the
overfitting observed past epoch 3--5, and, time permitting, add a
