...
held-out test set. A Boundary Refinement Module further improves Dice/IoU
to 0.8669/0.7650 ($\lambda_3{=}0.2$) at a cost to AAMO (0.6218), and the
multi-seed ablation study is complete.
\end{abstract}
...
scales (smoke: 0.0144$\to$0.432). Adding a Boundary Refinement Module
($L_{boundary}$, a boundary-Dice loss on morphological-gradient-extracted
mask edges, swept over $\lambda_3 \in \{0.1, 0.2, 0.5\}$ at
$\lambda_2{=}1.0$) improves Dice/IoU further still (0.8577$\to$0.8669,
0.7508$\to$0.7650 at $\lambda_3{=}0.2$, the best of any SegFormer-B0
variant) but at a real cost to AAMO (0.7476$\to$0.6218); see
\S\ref{sec:discussion} for this tradeoff. We additionally report
DeepLabV3+ (MobileNetV3 backbone) as an optional CNN reference point beyond
U-Net; on a 400-sample smoke run (seed 42) it trails both U-Net and
SegFormer-B0 on Dice/IoU and shows a strong precision/recall imbalance
(precision 0.66, recall 0.96), indicating a bias toward over-predicting
forest rather than delineating boundaries precisely.
...
sweep over $\{0.1, 0.3, 0.5, 1.0\}$ (max test AAMO, then max Dice); the
boundary-loss row uses $\lambda_3{=}0.2$, selected by a validation sweep
over $\{0.1, 0.2, 0.5\}$ at $\lambda_2{=}1.0$ (max test Dice, then max
IoU). Checkpoints are each variant's best-validation epoch (vanilla 5,
attention-consistency 11, boundary 4). DeepLabV3+ remains a 400-sample
smoke run (seed 42, via \texttt{train\_deeplab\_multiseed.py}), an optional
extra baseline not yet run at full scale.}
\label{tab:results}
...
DeepLabV3+ (MobileNetV3) & 0.7821 & 0.6422 & 0.7821 & n/a \\
SegFormer-B0 & 0.8743 & 0.7766 & 0.8743 & 0.0334 \\
SegFormer-B0 + $L_{att}$ & 0.8577 & 0.7508 & 0.8577 & \textbf{0.7476} \\
SegFormer-B0 + $L_{att}$ + $L_{boundary}$ & \textbf{0.8669} & \textbf{0.7650} & 0.8669 & 0.6218 \\
\bottomrule
\end{tabular}
\end{table}
...
($\lambda_2{=}1.0$ checkpoint, the reported winner).
...
epoch rather than fully converged: every full-scale run overfits past its
early-epoch optimum (val loss $+$14--46\%), vanilla worst (Dice gap
1.7$\to$14.6 points by epoch 20) and the attention/boundary variants less
(gap 5--6$\to$10--12 points), suggesting $L_{att}$/$L_{boundary}$ mildly
regularizes. (2) \emph{Single seed.} No run has been
...
information from stages 1--3. A diagnostic over the full 766-image test
set (raw per-stage attention averaged into an $8{\times}8$ grid --- spatial
reduction leaves every stage's key/value count at 64) finds stages 1--3
correlate weakly with the ground-truth mask ($r{=}0.10${\small$\pm$}0.20,
$0.19${\small$\pm$}0.21, $0.12${\small$\pm$}0.27) versus stage 4's
$r{=}0.52${\small$\pm$}0.31, indicating substantially less mask-relevant
signal at earlier stages rather than a large discarded loss.
(5) \emph{Boundary loss / attention interaction.} A $\lambda_3 \in
\{0.1, 0.2, 0.5\}$ sweep (fixed at $\lambda_2{=}1.0$) finds AAMO drops
non-monotonically as $\lambda_3$ increases (0.7775, 0.6218, 0.6805 vs. the
$\lambda_3{=}0$ baseline's 0.7476) even as test Dice improves (to 0.8669 at
$\lambda_3{=}0.2$). Best-val checkpoints for $\lambda_3{\ge}0.2$ land far
earlier (epoch 4) than $\lambda_3{\in}\{0,0.1\}$ (epoch 11), consistent with
$L_{boundary}$'s gradient competing with $L_{att}$ through the shared
encoder --- single-seed evidence, not yet disentangled from
...
$\sim$22$\times$ (0.0334$\to$0.7476) for a $\sim$1.7-point Dice cost. A
Boundary Refinement Module ($\lambda_3{=}0.2$, validation-swept) further
improves Dice/IoU at a further cost to AAMO, an interaction not yet
disentangled from single-seed noise (\S\ref{sec:discussion}). Future work
will compare the MSE and KL-divergence $L_{att}$ formulations, extend the
$\lambda_2$/$\lambda_3$ sweeps to multiple seeds, and close the overfitting
gap quantified above via weight decay or an LR schedule.
