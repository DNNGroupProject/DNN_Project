...
\textbf{Implementation details.} SegFormer-B0 variants are trained with
AdamW (lr $=6{\times}10^{-5}$), batch size 4 (vanilla) / 1 (attention
variant, due to the double-backpropagation memory cost), Gaussian target
smoothing $\sigma = 8$, $\lambda_2 = 0.3$, and $L_{att}$ using the MSE
formulation, seed 42. Both the MSE and KL-divergence formulations of
$L_{att}$ are implemented and unit-tested, but only MSE has been evaluated
on trained checkpoints so far. All code runs CPU-only in this preliminary
phase; the full-scale run uses an identical training loop on a GPU runtime,
changing only dataset size and epoch count.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/attention_drift_02_post_train.png}
...
(Grad-Rollout); attention after adding the Attention Consistency Loss. The
attention-consistency map is visibly broader and warmer over more of the
image, consistent with the large AAMO gain, but does not yet cleanly
separate canopy from non-forest by eye; the vanilla map is dominated by a
corner artifact rather than the motivating roads/shadows failure mode.
Genuine output of the trained checkpoints, not cherry-picked, from an
8-epoch/400-image smoke run.}
\label{fig:attn}
\end{figure*}
...
Figure~\ref{fig:attn} shows the qualitative attention-drift comparison does
not yet tell as clean a story as the AAMO number: the vanilla model's
attention exhibits a corner artifact rather than the motivating
roads/shadows failure mode, plausibly from undertraining, and warrants
revisiting once the full-scale model has properly converged.
...
\textbf{Threats to validity.} (1) \emph{Scale.} Results use 7.8\% of the
dataset (400/5{,}108 images) for 8 epochs; the direction of the AAMO gain
is unlikely to reverse, but its magnitude may shift once the model is
properly converged. (2) \emph{Single seed.} No run has been repeated with a
different seed, so the reported numbers carry unknown variance; Phase 2's
multi-seed ablation is required before any number here is treated as final.
(3) \emph{CNN baseline provenance.} The reported U-Net baseline's training
protocol is still being reconciled against the SegFormer runs' shared split
and training loop, and may be revised. (4) \emph{Rollout restriction.}
...
small-scale, single-seed results are not yet reliable enough to inform
real deforestation-monitoring decisions.
