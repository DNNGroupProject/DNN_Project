...
# ── FULL-DATASET TRAINING SCHEDULE (commented out while experimenting) ─────
# NUM_EPOCHS = 50
# LEARNING_RATE = 1e-3
# BCE_WEIGHT = 0.5
# DICE_WEIGHT = 0.5
...
# ── SUBSET / QUICK-EXPERIMENT TRAINING SCHEDULE (active) ───────────────────
# Fewer epochs + shorter patience so a failed experiment doesn't burn time.
# Flip USE_SUBSET to False and un-comment the block above for the real run.
USE_SUBSET = True
SUBSET_FRACTION = 0.15   # fraction of full dataset to use while experimenting
...
NOTE: while experimenting with a subset, this still copies the FULL folders from
Drive to local disk first (subsampling happens afterwards, on the path lists).
If Drive I/O itself is your bottleneck, see the optional note at the end of
Cell 3 below for copying only the subset's files instead.
"""
...
# Call the new function to get paths (FULL dataset — every valid pair on disk)
image_paths, mask_paths = load_dataset_paths(IMAGE_FOLDER, MASK_FOLDER)

# ── SUBSET SELECTION (active while experimenting) ──────────────────────────
# Shuffle deterministically (SEED) and slice BEFORE the train/val/test split
# below, so the 80/10/10 split, augmentation, and model code stay untouched —
# they simply see fewer pairs. Flip USE_SUBSET to False for the full run.
if USE_SUBSET:
...
# (image_paths / mask_paths here are already the SUBSET if USE_SUBSET=True,
# since the slicing happened above, before this split.)
